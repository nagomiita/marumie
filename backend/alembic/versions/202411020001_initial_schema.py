...
def upgrade() -> None:
    ...
    personal_transaction_type_enum = sa.Enum(
        "income", "expense", name="personal_transaction_type"
    )
    ...
    op.create_table(
        ...
        sa.Index(
            "ix_personal_transactions_org_date", "organization_id", sa.text("date DESC")
        ),
...
def downgrade() -> None:
    ...
    organization_type_enum = sa.Enum(
        "household", "business", "nonprofit", "other", name="organization_type"
    )
    user_role_enum = sa.Enum("admin", "user", name="user_role")
